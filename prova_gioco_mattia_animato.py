...
LARGHEZZA_TABELLONE = 1000
scala = LARGHEZZA_TABELLONE / larghezza_originale

Y_MESSAGGIO = altezza_originale * scala + 80
...
altezza_finestra = altezza_tabellone + MARGINE * 4
...
bottone_vero = pygame.Rect(pannello_x + 40, Y_MESSAGGIO-60, 120, 50)
bottone_falso = pygame.Rect(pannello_x + 190, Y_MESSAGGIO-60, 120, 50)
...
# Variabili per far muovere il pallino lungo il percorso,
# una cella alla volta, invece di saltare direttamente alla tappa successiva.
indice_percorso_attuale = percorso.index(posizione)
indice_percorso_arrivo = indice_percorso_attuale
sta_muovendo = False
...
while True:
    ...
    for evento in pygame.event.get():
        ...
        if evento.type == pygame.MOUSEBUTTONDOWN and not gioco_finito and not sta_muovendo:
            ...
            if risposta_data is not None:
                ...
                if risposta_data == risposta_corretta:
                    ...
                    if numero_tappa == len(tappe):
                        nuova_posizione = arrivo
                        indice_percorso_arrivo = percorso.index(nuova_posizione)
                        sta_muovendo = True
                        messaggio = "Hai vinto!"
                        gioco_finito = True
                    else:
                        nuova_posizione = tappe[numero_tappa]
                        indice_percorso_arrivo = percorso.index(nuova_posizione)
                        sta_muovendo = True
                    ...
                    # Se sbagli, torni alla tappa precedente.
                    # max(0, ...) evita di andare sotto la prima tappa.
                    numero_tappa = max(0, numero_tappa - 1)
    ...
    if sta_muovendo:
        if indice_percorso_attuale < indice_percorso_arrivo:
            indice_percorso_attuale += 1
        elif indice_percorso_attuale > indice_percorso_arrivo:
            indice_percorso_attuale -= 1
        else:
            sta_muovendo = False

        posizione = percorso[indice_percorso_attuale]


    # --------------------------------------------------------
    # 3. DISEGNO DEL TABELLONE
    # --------------------------------------------------------
    ...
    pygame.draw.lines(schermo, NERO, False, punti_percorso, 8)
    ...
    pygame.draw.circle(schermo,NERO, (centro_x, centro_y), 14)
    ...
    pygame.display.flip()
    # Numero più basso = movimento più lento.
    # Prova 5 per più lento, 15 per più veloce.
    orologio.tick(8)
